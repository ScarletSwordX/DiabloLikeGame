# 任务表（Diablo-like Demo）

**唯一排期来源**：阶段边界、系统任务、DoD、状态均在本表维护。

- 愿景：[GAME_CORE.md](./GAME_CORE.md)  
- 系统索引：[SYSTEM_DESIGN.md](./SYSTEM_DESIGN.md)  
- 程序架构：[ARCHITECTURE.md](./ARCHITECTURE.md)  
- 需求原文：[CHALLENGE.md](../CHALLENGE.md)

**任务 ID**：`P1` 阶段一 | `P2` 阶段二 | `P3` 阶段三 | `DLV` 交付 | `BASE` 已完成基线  

**约束**：MVP 架构；ScriptableObject 数据；Input System；**禁止 `git push`**。

**分类结构**：第一层 **阶段** → 第二层 **系统**（对应 `docs/systems/<SystemId>/`）。
...
| **二** | 内容 + 表现 + 拾取链路 | ≥3 技能/道具 SO；状态表扩展；击杀掉落 + 靠近拾取 + **简化库存**；敌人/玩家动画；CHALLENGE 五条 | 经验、解锁、完整背包 UI、自由配装 UI |
...
| 已有 | 差距 |
|------|------|
| Main 场景 Player/Enemy Prefab + 系统/HUD/拾取 Prefab | Bootstrap 不再运行时 `CreatePrimitive` |
| 指向 fireball + shield | 阶段一改为非指向 `nova`，收敛 1 技能 |
| 场景药水 + 快捷栏次数 | 阶段一改为即时拾取，无背包语义 |
| 假人不可死亡 | 需 `EnemyDiedEvent` |
| 固定 Bootstrap 装备 | 阶段三 `SkillLoadout` |
...
| DLV-AI-02 | `.gitignore` 不忽略 AI 目录 | 已检 | 完成 |
| DLV-02 | README §1–§6 与 CHALLENGE 对齐 | 含自测与 AI 协作说明 | 完成 |
| DLV-AI-01 | `.cursor/` 会话入库 | 提交前自检 | 完成 |
...
| ID | 任务 | DoD | 状态 |
|----|------|-----|------|
| P1-CHR-01 | 假人 HP 归零可死亡 | 发 `EnemyDiedEvent` | 待做 |
...
| ID | 任务 | DoD | 状态 |
|----|------|-----|------|
| P1-SKL-01 | 默认技能改为非指向 AOE（`nova`） | `Targeting.NonTargeted` + `AreaRadius` | 待做 |
| P1-SKL-02 | 收敛为 1 个战斗技能键 | 移除/禁用 shield 等 | 待做 |
| P1-SKL-03 | CHALLENGE 占位第三技能 SO | 仅配置存在，可不绑键 | 待做 |

### SkillLoadout — `docs/systems/SkillLoadout/`

| ID | 任务 | DoD | 状态 |
|----|------|-----|------|
| P1-LD-01 | **预留**装备槽数据结构 | `equippedSkillIds[]` 写死 Bootstrap | 待做 |
...
| P1-ITM-02 | 移除阶段一对快捷栏次数的依赖 | 不用 Item1–3 当背包 | 待做 |
...
| ID | 任务 | DoD | 状态 |
|----|------|-----|------|
| P1-EVT-01 | 新增 `EnemyDiedEvent` | Progression/Loot 可订阅 | 待做 |

### Input — `docs/systems/Input/`

| ID | 任务 | DoD | 状态 |
|----|------|-----|------|
| P1-INP-01 | 收敛阶段一输入 | 1 技能 + 移动 + 拾取 | 待做 |

### DebugTest — `docs/systems/DebugTest/`

| ID | 任务 | DoD | 状态 |
|----|------|-----|------|
| P1-DBG-02 | 自测覆盖死亡 / 非指向伤害 | 扩展用例 | 待做 |

### Docs

| ID | 任务 | DoD | 状态 |
|----|------|-----|------|
| P1-DOC-01 | GAME_CORE / 本表 / SYSTEM_DESIGN 对齐 | 文档评审 | 完成 |
...
| P2-CHL-01 | 复制 `HeavyFireball` 接入 | CHALLENGE 用例 4 | 待做 |

### SkillLoadout — `docs/systems/SkillLoadout/`

| ID | 任务 | DoD | 状态 |
|----|------|-----|------|
| P2-DOC-LD | 新建 `systems/SkillLoadout/SYSTEM.md` | 接口约定 | 待做 |

### Item — `docs/systems/Item/`

| ID | 任务 | DoD | 状态 |
|----|------|-----|------|
| P2-ITM-01 | ≥2 种道具 SO | 治疗、移速 Buff 等 | 待做 |

### Inventory — `docs/systems/Inventory/`

| ID | 任务 | DoD | 状态 |
|----|------|-----|------|
| P2-INV-01 | **简化库存**（列表/计数） | 拾取入列表，无格子 UI | 待做 |
| P2-DOC-INV | 新建 `systems/Inventory/SYSTEM.md` | 与 Loot 拾取约定 | 待做 |

### Loot — `docs/systems/Loot/`

| ID | 任务 | DoD | 状态 |
|----|------|-----|------|
| P2-Loot-01 | 敌人死亡生成掉落物 | 订阅 `EnemyDiedEvent` | 待做 |
| P2-Loot-02 | 靠近自动拾取 | 对接 `P2-INV-01` | 待做 |
| P2-DOC-Loot | 新建 `systems/Loot/SYSTEM.md` | 掉落物 Prefab 约定 | 待做 |

### Status — `docs/systems/Status/`

| ID | 任务 | DoD | 状态 |
|----|------|-----|------|
| P2-STA-01 | 扩展状态表与 `ActionEffectProfile` | 护盾/减速等 SO 条目 | 待做 |
| P2-DOC-STA | 补全 `Status/DATA.md` | 与任务对齐 | 待做 |

### Progression — `docs/systems/Progression/`

| ID | 任务 | DoD | 状态 |
|----|------|-----|------|
| P2-DOC-PRG | 新建 `systems/Progression/SYSTEM.md` | 经验/解锁设计 | 待做 |

### FeedbackUI — `docs/systems/FeedbackUI/`

| ID | 任务 | DoD | 状态 |
|----|------|-----|------|
| P2-UI-01 | Buff/掉落/简化库存提示（可选） | 扩展 HUD | 待做 |

### Feedback — `docs/systems/Feedback/`

| ID | 任务 | DoD | 状态 |
|----|------|-----|------|
| P2-FDB-01 | Feel 实现 `IGameplayFeedback` | 命中/治疗/拾取 VFX | 待做 |

### DebugTest — `docs/systems/DebugTest/`

| ID | 任务 | DoD | 状态 |
|----|------|-----|------|
| P2-CHL-01 | CHALLENGE 用例 1–5 全覆盖 | 评审清单 | 待做 |
...
| ID | 任务 | DoD | 状态 |
|----|------|-----|------|
| P3-PRG-01 | 击杀获得经验 | `XpGainedEvent`；订阅 `EnemyDied` | 待做 |
| P3-PRG-02 | 等级与 `SkillUnlockTable` SO | `Assets/Data/Progression/` | 待做 |
| P3-PRG-03 | 解锁通知 UI | HUD 或弹条 | 待做 |

### EventBus — `docs/systems/EventBus/`

| ID | 任务 | DoD | 状态 |
|----|------|-----|------|
| P3-EVT-01 | 新增 `SkillUnlockedEvent` 等 | UI 解锁通知 | 待做 |
...
| ID | 任务 | DoD | 状态 |
|----|------|-----|------|
| P3-INV-01 | 背包数据 + UI | 完整背包 | 待做 |

### Item — `docs/systems/Item/`

| ID | 任务 | DoD | 状态 |
|----|------|-----|------|
| P3-INV-02 | 从背包使用消耗品 | 替代快捷栏 | 待做 |

### SkillLoadout — `docs/systems/SkillLoadout/`

| ID | 任务 | DoD | 状态 |
|----|------|-----|------|
| P3-LD-01 | 已解锁技能 → 技能栏自由配置 | UI + Presenter | 待做 |

### Loot — `docs/systems/Loot/`

| ID | 任务 | DoD | 状态 |
|----|------|-----|------|
| P3-Loot-01 | `LootTable` SO 驱动掉落 | 按敌人配置 | 待做 |

### FeedbackUI — `docs/systems/FeedbackUI/`

| ID | 任务 | DoD | 状态 |
|----|------|-----|------|
| P3-UI-01 | 背包 UI + 技能配置 UI | 与 Inventory/Loadout 联动 | 待做 |
...
| 用例 | 说明 | 对应任务 | 最早阶段 |
|------|------|----------|----------|
| 1 | 改配置改表现 | P1-SKL-01，BASE-EFT | 一 |
| 2 | 冷却连按 + UI | BASE-SKL-02，BASE-UI | 一 |
| 3 | 拾取/消耗 | P1-ITM-01；三 b `P3-INV-*` | 一 / 三 b |
| 4 | 复制 HeavyFireball | P2-CHL-01 | 二 |
| 5 | 自测路径 | BASE-DBG-01，P1-DBG-02 | 一 |
| ≥3 配置项 | 阶段一仅 2 玩法项 | **P1-SKL-03** 占位 SO | 一 / 二 |
